...
class CinetPay
{
  ...
  {
    // dd('route-cinetpay-ipn', route('cinetpay-ipn'));
    $this->notify_url = route('cinetpay-ipn');
    $this->apikey = config('cinetpay.CINETPAY_API_KEY');
    $this->site_id = config('cinetpay.CINETPAY_SITE_ID');
  ...
  public function CheckDataExist($param, $action) // a customiser pour la function check status
  {
    ...
    if ($action == "payment") {
      ...
      if (empty($param['notify_url']) && empty($this->notify_url))
  ...
  public function getPayStatus($id_transaction, $site_id)
  {
    ...
    return [
      'code' => $StatusPayment['code'],
      'message' => $StatusPayment['message'],
      'amount' => $StatusPayment['data']['amount'],
      'metadata' => $StatusPayment['data']['metadata'],
      'currency' => $StatusPayment['data']['currency'],
      'operator_id' => $StatusPayment['data']['operator_id'],
      'api_response_id' => $StatusPayment['api_response_id'],
      'description' => $StatusPayment['data']['description'],
      'payment_date' => $StatusPayment['data']['payment_date'],
      'payment_method' => $StatusPayment['data']['payment_method'],
    ];
  }
